...
dts = logspace(-4.5, -4.8, 4);   % example: [1e-3, 1e-3.666..., 1e-4.333..., 1e-5]
for i = 1:length(dts)
    dt = dts(i);
    % set nsteps so total time ~ 2*pi
    nsteps = round(2*pi / dt);
    % call dt_chk (function) and collect results
    [kN,kdt,kem,ktm,ktrial] = dt_chk(dt, nsteps, kN, kdt, kem, ktm, ktrial);
end
...
% Fit a line to log-log data to estimate order (slope)
% Only fit points with positive error
ok = kem > 0 & kdt > 0;
if sum(ok) >= 2
    p = polyfit(log10(kdt(ok)), log10(kem(ok)), 1);
    slope = p(1);
    hold on;
    % display fitted slope on the plot
    txt = sprintf('fit slope = %.2f', slope);
    xpos = 10^(mean(log10(kdt(ok))));
    ypos = 10^(mean(log10(kem(ok))));
    text(xpos, ypos, txt, 'HorizontalAlignment', 'center', 'BackgroundColor', 'w');
end
